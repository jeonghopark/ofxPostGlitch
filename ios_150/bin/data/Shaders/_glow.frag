...
//    }
   ...
   for (int i = -blur_w;i < blur_w;i++){
      for (int j = -blur_w;j < blur_w;j++){
        weight = pow(1.0/2.0*pi*k*k,-((float(i*i)+float(j*j))/2.0*k*k))/(float(blur_w)+1.0);
        gws = gws + texture2D(tex0,vec2(pos.x+float(j),pos.y+float(i)))*weight*1.3;
        weight = pow(1.0/2.0*pi*k*k,-((float(i*i)+float(j*j))/2.0*k*k))/(float(blur_w)+1.0);
        gws = gws + texture2D(tex0,vec2(pos.x+float(j),pos.y+float(i)))*weight*1.3;
        weight = pow(1.0/2.0*pi*k*k,-((float(i*i)+float(j*j))/2.0*k*k))/(float(blur_w)+1.0);
        gws = gws + texture2D(tex0,vec2(pos.x+float(j),pos.y+float(i)))*weight*1.3;
        // gws = gws + texture2D(tex0,vec2(varyingtexcoord.x+float(mod(float(i),float(blur_w))),varyingtexcoord.y+float(i/blur_w)))*weight*1.3;
        // gws = gws + texture2D(tex0,vec2(varyingtexcoord.x-float(mod(float(i),float(blur_w))),varyingtexcoord.y+float(i/blur_w)))*weight*1.3;
        // gws = gws + texture2D(tex0,vec2(varyingtexcoord.x+float(mod(float(i),float(blur_w))),varyingtexcoord.y-float(i/blur_w)))*weight*1.3;
        // gws = gws + texture2D(tex0,vec2(varyingtexcoord.x-float(mod(float(i),float(blur_w))),varyingtexcoord.y-float(i/blur_w)))*weight*1.3;
    }
    ...
    gl_FragColor.rgba = col + gws.rgba;
}
